...
<%   /////////////falta tirar a comunidade lá de cima
	String communityID = request.getParameter("communityID");
	
	Community viewedCommunity;
	/* Ele pode esta recebendo dois tipos de parametros, ou uma communityID ou a comunidade ja esta na sessao. */
	if (communityID==null){
		viewedCommunity = community;
	}else{
		viewedCommunity = AbstractCommunity.getRealCommunity(communityID,database);
	}
%>
<TITLE>graW - Gradua&ccedil;&atilde;o na Web - TCI/UFAL - Informações sobre a comunidade <%= viewedCommunity.getTitle() %></TITLE>
<!-- #EndEditable -->
      ...
      <TR> 
        <TD class=menuFora height=0 valign="top"> 
          <table cellspacing=0 cellpadding=0 width=760 border=0 align="left" height="0">
                          ...
                          <tr> 
                            <td height="0"> 
                              <div align="right"><!-- #BeginEditable "directory" --><b>Minhas 
								...
								<%
			if (viewedCommunity instanceof Group){
				Course course = ((Group)viewedCommunity).getCourse() ;
				if (course!=null){	%>
			...
			%>
								<!--<a href=information.jsp><%=viewedCommunity.getTitle() %></a>  -->
								<font class=menuFora>&#149;</font> Informa&ccedil;&otilde;es</b><!-- #EndEditable --></div>
                            </td>
                          </tr>
                          <tr> 
...
<BR><center>
                        <table class=tabelaSimples cellspacing=0 cellpadding=3 width="92%">
                          <tbody> 
			  <tr>
			  	<td id=headerSimples><%=viewedCommunity.getCategoryDescription()%> <font class=menuFora>  &#149;</font> <%=viewedCommunity.getSubcategoryDescription()%> </td>
			  </tr>
                          <tr> 			  
        	                    <td id=headerSimplesFont><%=viewedCommunity.getTitle() %> 
				    </td>
                          </tr>
                          <tr> 
	                            <td id=corpoSimples height="0"><%=viewedCommunity.getDescription()%></td>
                          </tr> 
			  <!--  Se for um curso --> <%
			  if (viewedCommunity instanceof Course){
			  	isCourse=true;	%>
			  	<tr> 
					<td id=headerSimplesFont>Carga Horária: </td>
				</tr>
                          	<tr> 
                            		<td id=corpoSimples height="0"><%=((Course)viewedCommunity).getHours()%></td>
				</tr>				
				<tr> 
					<td id=headerSimplesFont>Programa: </td>
				</tr>
                          	<tr> 
                            		<td id=corpoSimples height="0"><%=Utility.getTextField(  ((Course)viewedCommunity).getProgram() )%></td>
				</tr>			  					
				<tr> 
					<td id=headerSimplesFont>Bibliografia: </td>
				</tr>
                          	<tr> 		
				      <td id=corpoSimples height="0"><%=Utility.getTextField(  ((Course)viewedCommunity).getBibliography() )%></td>
				</tr> <%
				if (viewedCommunity instanceof Discipline ) {
					discipline=true;	%>
					<tr> 
						<td id=headerSimplesFont>Código da disciplina: </td>
					</tr>
                        	  	<tr> 
                            			<td id=corpoSimples height="0"><%=((Discipline)viewedCommunity).getCode()%></td>
					</tr>	<%
				}
			  } %>			  
