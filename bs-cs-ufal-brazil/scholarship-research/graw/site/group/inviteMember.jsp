  ...
  <DIV align=center><br>
    <TABLE class="tabelaMaior" cellSpacing=0 cellPadding=0 width=748 border="0" height="0">
      ...
      border=0>
		...
		<br>
		  <form action="../servlet/br.ufal.graw.web.user.InviteUserToGroup" method="post">
            <div align=center>
			<table width="100%" border="0" class="tabelaSimples">
					<tr>
						
                  <td id=headerSimples> Selecione os usu&aacute;rios que você 
                    deseja convidar ao grupo: <%=group.getName() %> </td>
					</tr>
					<%
					Enumeration members = course.getMembers();
					User person;
					int total = 0;					
					while ( members.hasMoreElements() ){
						person = (User) members.nextElement();
						if (  !  group.isMember(person)  ){	
							total++;	%>
							<tr>						
				                <td id=corpoSimples> 
        					          <input type="checkbox" name="userID" value="<%=person.getID()%>"><%=person.getName()%>
									  <a href="../email/index.jsp?userID=<%=person.getID()%>" > <img src="../sources/images/email.gif" border=0> </a>
					            </td>
							</tr>	<%
						}
					}
					if 	(total==0){		%>
						<tr>						
				         	<td id=corpoSimples> 
        						Não existe mais ninguém para convidar que esteja  associado ao curso "<%=course.getName()%>". <br>
								Você só pode convidar pessoas para esse grupo que estejam associadas com o curso "<%=course.getName()%>".
					         </td>
						</tr>	<%
					}%>
					<tr>						
                  ...
                  <td id=corpoSimples> 
                    <div align="center">
                      <input type="submit" name="Submit" value="Convidar" class="botao">
                    </div>
                  </td>
					</tr>
			</table>
