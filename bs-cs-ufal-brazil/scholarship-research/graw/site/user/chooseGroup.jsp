    ...
    <TABLE class="tabelaMaior" cellSpacing=0 cellPadding=0 width=748 border="0" height="0">
      ...
      border=0>
		...
		<table class=tabelaSimples cellspacing=1 cellpadding=3 width="100%">
          ...
          no nome de um dos grupos para acessar seus serviços e informações. <br>
          <BR>
          <center>
            <TABLE class=tabelaSimples cellSpacing=0 cellPadding=3 width="61%">
              ...
              <%  
                ...
                <TD id=headerSimples height="0"><b>Grupos :</b></TD>
    	        </TR>
				<%
				while (groups.hasMoreElements()){			
					color = ((counterGroups% 2)== 0) ? "ffffff" : "f5f5f5";
					counterGroups++;					
					groupItem= (Group)groups.nextElement();
							%>
						...
						<TR onmouseover="mOvr(this, '#e0e0e0'); " onmouseout="mOut(this, '#<%= color %>'); "  bgColor=#<%= color %>>
		    	            <TD height="0">&nbsp;<img src="../sources/images/seta.gif" width="11" height="11"> 
        			          <a  class="Link" href="../servlet/br.ufal.graw.web.group.OpenGroup?groupID=<%= groupItem.getID() %>"><%=groupItem.getName()%></a>
							  - ( <%= (groupItem.getCourse()).getName() %> )
							  	<%
							  	if ( groupItem.isAdministrator(user ) ){ 	%>
										<img src="../sources/images/administrator.gif" >	<%									
								}	
								%>
							</TD>
		              </TR>	<%
				...
				if (counterGroups == 0){	%>
					<TR >
		    	            <TD id=corpoSimples >
							Você não está associado a nenhuma grupo.
							</TD>
		              </TR>	<%
				}	%>
				<tr>
				...
				<td id=headerSimples>Legendas</td>
								...
								<tr>
				<td><img src="../sources/images/administrator.gif" > - Administra</td>
				</tr>
              </TBODY> 
            </TABLE>
          </center>
