...
public class UserCommunityPreferences{
	...
	private Vector communityPerfils; //CommunityPerfil
	...
	public UserCommunityPreferences(User user, Community community) throws PerfilException{
		if (!user.isMember(community))
			throw new PerfilException("Impossível carregar Perfil de Usuário ("+user.getID()+") para a comunidade ("+community.getID()+")!");
		else {
			this.user = user;
	...
	/**
	 * Method hasChosen. Verifica se um dado perfil foi escolhido pelo usuário.
	 *
	 ...
	 * que o usuário tem na comunidade. [perfilID, true]
	 *
	 */
	private void getUserPerfilsRelation(){
		...
		for (int i = 0; i < quant; i++){
			...
			try{ //falta fazer tb com a logica do vetor...
				UserCommunityPerfil ucp = new UserCommunityPerfil(perfilUserID,this.database);
				if (!CommunityPerfil.exists(this.community.getID(),ucp.getPerfilID(),this.database))
					this.removeOldPreference(ucp.getPerfilID());
				else {
	...
	/**
	 * Method removeOldPreference. Remove preferencias do usuário que ainda existe
	 * em seu perfil, porém o responsável pela comunidade atualizou o perfil.
	 *
	 * @param    perfilID            a  String
	 *
	 */
