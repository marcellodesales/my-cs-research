...
public class Config {
	...
	private Hashtable messenger;
	
	private FileReader configurationFile;
	private BufferedReader readerOfFile;
	private Hashtable hash;
	private Vector variables;
	private final String TXT_CONF = "c:/desenv/graw/config/graW.txt";
//	private final String XML_CONF = "z:/desenv/nonamepibic/config/graW.xml";
	private final String XML_CONF = "c:/desenv/graw/config/graW.xml";
	...
	private void initializeXMLConf(){
		try {
			...
			org.jdom.Document config = saxBuilder.build(new File("c:/desenv/graw/config/graW.xml"));
			this.parseConfigFile(config);
		} catch (JDOMException jde){
			System.err.println("JDOMException: " + jde.getMessage());
	...
	private void parseWebsite(Element website){
		...
		Element messenger = website.getChild("messenger");
		this.messenger.put("socket", messenger.getChild("socket").getText());
	}
	...
	public String getDatabaseURL(){
		return (String)((Hashtable)this.databases.elementAt(0)).get("local");
	}
	
	public String getDatabaseDriver(){
		return (String)((Hashtable)this.databases.elementAt(0)).get("driver");
	}

	public String getDatabaseUsername(){
		return (String)((Hashtable)this.databases.elementAt(0)).get("username");
	}
	
	public String getDatabasePassword(){
		String a = (String)((Hashtable)this.databases.elementAt(0)).get("password");
	...
	public String getMessenger(String key) {
		return this.messenger.get(key).toString();
	}
