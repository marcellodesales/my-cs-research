...
<title>Cadastro de exemplar - BibliWEB</title>
<!-- #EndEditable -->
...
<table class="tabelaMaior" width="770" border="0" cellspacing="2" cellpadding="1" align="center" height="426">
  ...
  <tr class="escuro"> 
    <td height="21" width="652"><font color=white size=2 face=verdana><b><!-- #BeginEditable "title" -->Cadastro 
      de exemplar :: Tipo de Exemplar<!-- #EndEditable --></b></font></td>
...
%>
      <br>
      <form action="newFormExemplar.jsp" method="POST">
        <table width="520" border="0" align="center" cellpadding="1" cellspacing="1">
          <tr> 
            <td id=headerSimplesFont>Escolha o tipo do exemplar<b></b></td>
          </tr>
          <tr> 
            <td id=corpoSimples>&nbsp;&nbsp; 
              <select name='exemplarTypeID' style="background-color: #FFCC00">
<%
		ExemplarType[] exemplarTypes = ComponentSet.getExemplarTypes(databaseLayer);
		for (int i=0; i < exemplarTypes.length; i++){ %>
                <option value='<%=exemplarTypes[i].getID()%>'><%=exemplarTypes[i].getDescription()%></option>
<%		}
%>
              </select>
              &nbsp;&nbsp; </td>
