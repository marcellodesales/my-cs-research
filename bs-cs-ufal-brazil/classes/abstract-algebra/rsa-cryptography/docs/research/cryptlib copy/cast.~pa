...
{
Cast is a 128-bit block algorithm with vairiable key
sizes up to 256-bits;
}

type
  array4 = array[0..3] of integer;
  arraytmr = array[0..23,0..7] of integer;
  array8 = array[0..7] of integer;
  arraymr = array[0..11,0..3] of integer;

  TCast = class(TComponent)
  private
     ikey : string;
     IFile : string; {input file}
     OFile : string; {output file}
     procedure expandkey(uk : array8);
     {Main functions}
     function f1(a,mk,rk : integer): integer;
     function f2(a,mk,rk : integer): integer;
     function f3(a,mk,rk : integer): integer;
     function Q(g : array4; i : integer): array4;
     function RQ(g : array4; i : integer): array4;
     function W(g : array8; i : integer; tr,tm : arraytmr): array8;
     {encrypt\decrypt block functions}
     function encryptblock(g : array4): array4;
     function decryptblock(g : array4): array4;
  public
...
$14f03084, $066ff472, $a31aa153, $dadc4755, $b5625dbf, $68561be6, $83ca6b94, $2d6ed23b,
$eccf01db, $a6d3d0ba, $b6803d5c, $af77a709, $33b4a34c, $397bc8d6, $5ee22b95, $5f0e5304,
$81ed6f61, $20e74364, $45e13789, $de18639b, $881ca122, $b96726d1, $8049a7e8, $22b7da7b,
...
procedure Register;
begin
  RegisterComponents('Crypto', [TCast]);
end;
...
function TCast.f1(a,mk,rk : integer ): integer;
var
I : integer;
ib : array[1..4] of byte absolute I;
begin
I := rotl(mk+a,rk);
f1 := ((S1[ib[1]] xor S2[ib[2]])- S3[ib[3]])+ S4[ib[4]];
end;

{*****************************************}
{*****************************************}

function TCast.f2(a,mk,rk : integer): integer;
var
I : integer;
ib : array[1..4] of byte absolute I;
begin
I := rotl(mk xor a,rk);
f2 := ((S1[ib[1]] - S2[ib[2]])+ S3[ib[3]])xor S4[ib[4]];
end;

{*****************************************}
{*****************************************}

function TCast.f3(a,mk,rk : integer): integer;
var
I : integer;
ib : array[1..4] of byte absolute I;
begin
I := rotl(mk-a,rk);
f3 := ((S1[ib[1]] + S2[ib[2]])xor S3[ib[3]])- S4[ib[4]];
end;

{*****************************************}
{*****************************************}

function TCast.Q(g : array4; i : integer): array4;
var
A,B,C,D : integer;
begin
A := g[0];B := g[1];
...
C := C xor f1(D,kr[i,0],km[i,0]);
B := B xor f2(C,kr[i,1],km[i,1]);
A := A xor f3(B,kr[i,2],km[i,2]);
D := D xor f1(A,kr[i,3],km[i,3]);
...
function TCast.RQ(g : array4; i : integer): array4;
var
A,B,C,D : integer;
begin
A := g[0];B := g[1];
...
function TCast.W(g : array8; i : integer; tr,tm : arraytmr): array8;
var
A,B,C,D,E,F,GG,H : integer;
begin
A := g[0];B := g[1];
C := g[2];D := g[3];
E := g[4];F := g[5];
...
GG := GG xor f1(H,tr[i,0],tm[i,0]);
F := F xor f2(GG,tr[i,1],tm[i,1]);
E := E xor f3(F,tr[i,2],tm[i,2]);
D := D xor f1(E,tr[i,3],tm[i,3]);
C := C xor f2(D,tr[i,4],tm[i,4]);
B := B xor f3(C,tr[i,5],tm[i,5]);
A := A xor f1(B,tr[i,6],tm[i,6]);
H := H xor f2(A,tr[i,7],tm[i,7]);
...
procedure TCast.expandkey(uk : array8);
var
i,j,cm,mm,cr,mr : integer;
k : array8;
tm,tr : arraytmr;
begin
cm := $5a827999;
...
function TCast.encryptblock(g : array4): array4;
var
i : integer;
begin
g := Q(g,0);g := Q(g,1);
...
function TCast.decryptblock(g : array4): array4;
var
i : integer;
begin
g := Q(g,11);g := Q(g,10);
...
function TCast.testcipher: string;
var
i : integer;
arr : array4;
uk : array8;
...
move(Ikey,uk,sizeof(uk));

expandkey(uk);
arr := encryptblock(arr);
arr := decryptblock(arr);
