...
void init_des()
  { int comes_from_bit;
    ...
    for(bit = 0; bit < 64; bit++) {
      int o_bit, o_long;
      unsigned word_value, mask1, mask2;
...
static void shuffle_sb(k, saltbits)
  ...
  { unsigned j;
    unsigned long x;
    for(j=4096; j--;) {
      x = (k[0] ^ k[1]) & (unsigned long)saltbits;
      *k++ ^= x;
      *k++ ^= x;
    }
  }
...
static unsigned current_saltbits = 0;
static int direction = 0;

static void setup_salt(s)
  char *s;
  { unsigned i, j, saltbits;
...
static void ufc_mk_keytab(key)
  ...
  { unsigned v1, v2, *k1;
    int i;
    unsigned long v, *k2 = &ufc_keytab[0][0];
...
unsigned *ufc_dofinalperm(l1, l2, r1, r2)
  unsigned l1,l2,r1,r2;
  { unsigned v1, v2, x;
    static unsigned ary[2];
...
extern unsigned *ufc_dofinalperm();

/*
 * 32 bit version
 */

extern unsigned long ufc_keytab[16][2];
extern unsigned long ufc_sb0[], ufc_sb1[], ufc_sb2[], ufc_sb3[];

#define SBA(sb, v) (*(unsigned long*)((char*)(sb)+(v)))

unsigned *ufc_doit(l1, l2, r1, r2, itr)
  unsigned l1, l2, r1, r2, itr;
  { int i;
    unsigned long s, *k;
...
char *crypt(key, salt)
  ...
  { unsigned *s;
     ...
     */
    s = ufc_doit((unsigned)0, (unsigned)0, 
		 (unsigned)0, (unsigned)0, (unsigned)25);
