...
public abstract class SessionResultsXMLHandler {
	...
	public static SessionResults parseSessionResults(Session currentSession, KXmlParser parser) throws XmlPullParserException, IOException{
         ...
         while ((eventType = parser.next()) != KXmlParser.END_DOCUMENT) {
             //only parser the results part, When the event is an start tag.
             if(eventType == KXmlParser.START_TAG) {   
                 if (parser.getName().equals("results"))
                     opened = parser.getAttributeValue("","opened").equals("true");
                 else
 	             if (parser.getName().equals("question-item"))
	                 sr.addVoteItem(new VoteItem(currentSession.getQuestions()[0].getQuestionItemString(parser.getAttributeValue("","id")),parser.getAttributeValue("","number"),parser.getAttributeValue("","percentage")));
 	         }	 
         }
         return sr;
	 }
